 ...
 *          auto mc = std::make_shared<mcpele::MC>(pot, SomeCoordinates,
 * SomeTemperature); auto step_pattern =
 * std::make_shared<mcpele::TakeStepPattern>(); auto step_A =
 * std::make_shared<SomeTakeStep>(SomeParameters); auto step_B =
 * std::make_shared<SomeOtherTakeStep>(SomeOtherParameters);
...
class TakeStepPattern : public TakeStep {
 ...
 public:
  ...
  void report(pele::Array<double> &old_coords, const double old_energy,
    ...
    m_step_storage.at(m_steps.get_step_ptr())
  ...
  std::vector<size_t> get_pattern_direct() {
    return m_steps.get_pattern_direct();
  }
  const std::vector<size_t> get_changed_atoms() const {
    return m_step_storage.at(m_steps.get_step_ptr())->get_changed_atoms();
  }
  const std::vector<double> get_changed_coords_old() const {
    return m_step_storage.at(m_steps.get_step_ptr())->get_changed_coords_old();
  }
};
