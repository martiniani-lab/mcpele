...
/**
 * Monte Carlo
 * _coords and _trialcoords are arrays that store coordinates and trial
 * coordinates respectively _potential is on object of Pele::BasePotential type
 * that defines the interaction potential _E_reject_count is the count of
 * rejections due to an energy test (e.g. Metropolis) _conf_reject_count is the
 * count of rejections due to a configuration test (e.g. spherical container)
 * _niter is the count of steps whithin a MCMC run, it is reset to zero at the
 * end of the run _nitercount is the cumulative number of MCMC steps taken by
 * the class _neval is the number of energy evaluations _temperature is the
 * temperature at which the simulation is performed _energy is the current
 * energy of the system _success records whether the step has been accepted or
 * rejected
 */
...
public:
  ...
  const std::vector<std::shared_ptr<AcceptTest>> &display_accept_tests() const {
    return m_accept_tests;
  }
  ...
  double get_E_rejection_fraction() const {
    return static_cast<double>(m_E_reject_count) /
           static_cast<double>(m_nitercount);
  }
