...
pele::Array<double> mcpele::CheckHypercubicContainerConfig::gmc_gradient(
  ...
  if (not use_powered_cosine_sum_gradient) {
    ...
    assert(min_index < coords.size() && min_distance > 0.0 &&
           !std::isinf(min_distance));
    if (positive) {
  ...
  pele::PoweredCosineSum powered_cosine_sum(coords.size(), side_length, 0.5,
                                            coords.size());
