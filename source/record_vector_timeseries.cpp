...
RecordVectorTimeseries::RecordVectorTimeseries(const size_t record_every,
                                               const size_t eqsteps)
    : m_record_every(record_every), m_eqsteps(eqsteps) {
  if (record_every == 0) {
    throw std::runtime_error(
        "RecordVectorTimeseries: record_every expected to be at least 1");
  }
}
