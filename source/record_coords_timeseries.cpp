 ...
 * m_mcv2 element wise square mean coordinate vector, useful to compute the
 * variance in each coordinate
 * */
RecordCoordsTimeseries::RecordCoordsTimeseries(const size_t ndof,
                                               const size_t record_every,
                                               const size_t eqsteps)
    : RecordVectorTimeseries(record_every, eqsteps),
      m_mcv(ndof, 0),  // it is important that m_mcv is initialised to vec{0}
                       // for the first update step to work correctly
      m_mcv2(ndof, 0),
      m_ndof(ndof),
      m_count(0) {}
...
void RecordCoordsTimeseries::m_update_mean_coord_vector(
  ...
  if (counter > m_eqsteps) {
    ...
    if (counter % m_record_every == 0) {
      m_record_vector_value(
          this->get_recorded_vector(coords, energy, accepted, mc));
    }
  }
}
