...
GaussianTakeStep::GaussianTakeStep(const size_t rseed, const double stepsize,
                                   const size_t ndim)
    : m_seed(rseed),
      m_mean(0.0),
      m_stdev(1.0),
...
GaussianCoordsDisplacement::GaussianCoordsDisplacement(const size_t rseed,
...
void GaussianCoordsDisplacement::displace(pele::Array<double> &coords, MCBase *mc) {
  ...
  for (size_t i = 0; i < m_ndim; ++i) {
    coords[i] += m_normal_vec[i] *
                 m_stepsize;  // here the stepsize plays the same role as the
                              // stdev. This is sampled from N(0,stepsize)
  }
  ++m_count;
}

SampleGaussian::SampleGaussian(const size_t rseed, const double stepsize,
