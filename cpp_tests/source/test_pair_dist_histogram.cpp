...
TEST_F(TestPairDistHist, BasicFunctionality) {
  std::shared_ptr<mcpele::MC> mc =
      std::make_shared<mcpele::MC>(potential, x, 1);
  mc->set_takestep(step);
  const size_t eqsteps(max_iter / 1e1);
  std::shared_ptr<mcpele::RecordPairDistHistogram<boxdim>> record_gr =
      std::make_shared<mcpele::RecordPairDistHistogram<boxdim>>(
          boxvector, nr_bins, eqsteps, 1);
  std::shared_ptr<pele::GradientOptimizer> opt =
      std::make_shared<pele::LBFGS>(potential, x);
  std::shared_ptr<mcpele::RecordPairDistHistogram<boxdim>> record_gr_quench =
      std::make_shared<mcpele::RecordPairDistHistogram<boxdim>>(
          boxvector, nr_bins, eqsteps, 1, opt);
  ...
  // normalization OK
  if (boxdim == 2) {
    EXPECT_DOUBLE_EQ(gr[0], nparticles * (nparticles - 1) /
                                (nparticles * number_density * 2 * M_PI * r[0] *
                                 (boxlength * 0.5 / nr_bins)));
  }
  if (boxdim == 3) {
    const double dr = (boxlength * 0.5 / nr_bins);
    EXPECT_DOUBLE_EQ(gr[0],
                     nparticles * (nparticles - 1) /
                         (nparticles * number_density * 4 * M_PI / 3 *
                          (pow(r[0] + 0.5 * dr, 3) - pow(r[0] - 0.5 * dr, 3))));
  }
