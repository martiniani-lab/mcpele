...
TEST_F(TakeStepTest, Single_BasicFunctionalityAveragingErasing_OneIteration) {
  // one iteration gives expected variation
  mcpele::RandomCoordsDisplacementSingle displ(seed, nparticles, ndim,
                                               stepsize);
  displ.displace(coords, mc);
  for (size_t i = 0; i < ndof; ++i) {
    EXPECT_NEAR(reference[i], coords[i], stepsize * 0.5);
...
TEST_F(TakeStepTest, Single_BasicFunctionality_AllParticlesSampledUniformly) {
  ...
  EXPECT_NEAR_RELATIVE(hist_uniform_single.get_mean(), (nparticles - 1) / 2,
                       nparticles / sqrt(ntot));
  EXPECT_NEAR_RELATIVE(hist_uniform_single.get_variance(),
                       (ntot * ntot - 1) / 12,
                       (ntot * ntot - 1) / (12 * sqrt(ntot)));
}
...
TEST_F(TakeStepTest, UniformCubic_CorrectMoments) {
  ...
  std::vector<std::shared_ptr<mcpele::Histogram>> hist(
      100, std::make_shared<mcpele::Histogram>(0, nparticles - 1, 1));
  ...
  for (size_t i = 0; i < ndim; ++i) {
    ...
    EXPECT_NEAR_RELATIVE(hist[i]->get_variance(), delta * delta / 3,
                         2 / sqrt(nsamples));
  }
}
...
TEST_F(TakeStepTest, UniformSpherical_CorrectMoments) {
  ...
  EXPECT_NEAR_RELATIVE(pele::pos_int_pow<4>(radius) / 12, hist.get_variance(),
                       1e-2);
   ...
   * 12 * R^4 / 175.
   */
  EXPECT_NEAR_RELATIVE(3 * pele::pos_int_pow<2>(radius) / 5, hist3.get_mean(),
                       1e-3);
  EXPECT_NEAR_RELATIVE(12 * pele::pos_int_pow<4>(radius) / 175,
                       hist3.get_variance(), 1e-2);
  ...
  const double var_displ2_rw =
      1. / 180. * (pele::pos_int_pow<4>(lx) + pele::pos_int_pow<4>(ly));
...
TEST_F(TakeStepTest, PairSwapWorks) {
  ...
  for (size_t i = 0; i < ndof; ++i) {
    if (i >= a * box_dimension && i < (a + 1) * box_dimension) {
      EXPECT_DOUBLE_EQ(coords_1[i],
                       coords[(i + b * box_dimension) - a * box_dimension]);
    } else if (i >= b * box_dimension && i < (b + 1) * box_dimension) {
      EXPECT_DOUBLE_EQ(coords_1[i],
                       coords[(i + a * box_dimension) - b * box_dimension]);
    } else {
...
TEST_F(TakeStepTest, TakeStepProbabilities_Correct) {
  ...
  EXPECT_NEAR(
      freq0,
      static_cast<double>(static_cast<tt *>(ts0.get())->get_call_count()) /
          static_cast<double>(total_iterations),
      2e-3);
  EXPECT_NEAR(
      freq1,
      static_cast<double>(static_cast<tt *>(ts1.get())->get_call_count()) /
          static_cast<double>(total_iterations),
      2e-3);
  EXPECT_NEAR(
      freq2,
      static_cast<double>(static_cast<tt *>(ts2.get())->get_call_count()) /
          static_cast<double>(total_iterations),
      2e-3);
}
...
TEST_F(TakeStepTest, TakeStepPattern_Correct) {
  ...
  std::cout << "ts0->get_call_count(): "
            << static_cast<tt *>(ts0.get())->get_call_count() << "\n";
  std::cout << "ts1->get_call_count(): "
            << static_cast<tt *>(ts1.get())->get_call_count() << "\n";
  std::cout << "ts2->get_call_count(): "
            << static_cast<tt *>(ts2.get())->get_call_count() << "\n";
  ...
  EXPECT_NEAR(
      ...
      static_cast<double>(static_cast<tt *>(ts2.get())->get_call_count()) /
          static_cast<double>(total_iterations),
      2e-3);
  std::cout << "ts0->get_report_count(): "
            << static_cast<tt *>(ts0.get())->get_report_count() << "\n";
  std::cout << "ts1->get_report_count(): "
            << static_cast<tt *>(ts1.get())->get_report_count() << "\n";
  std::cout << "ts2->get_report_count(): "
            << static_cast<tt *>(ts2.get())->get_report_count() << "\n";
  std::cout << "report_iterations: " << report_iterations << "\n";
  EXPECT_NEAR(
      freq0,
      static_cast<double>(static_cast<tt *>(ts0.get())->get_report_count()) /
          static_cast<double>(report_iterations),
      1e-2);
  EXPECT_NEAR(
      freq1,
      static_cast<double>(static_cast<tt *>(ts1.get())->get_report_count()) /
          static_cast<double>(report_iterations),
      1e-2);
  EXPECT_NEAR(
      freq2,
      static_cast<double>(static_cast<tt *>(ts2.get())->get_report_count()) /
          static_cast<double>(report_iterations),
      1e-2);
}
...
TEST_F(AdaptiveTakeStepTest, Global_UniformAdaptive_Works) {
  ...
  auto uni = std::make_shared<mcpele::RandomCoordsDisplacementAll>(
      42, initial_stepsize);
  ...
  auto uni_adaptive = std::make_shared<mcpele::AdaptiveTakeStep>(
      uni, interval, factor, min_acc, max_acc);
  ...
  const double eq_acc_ratio =
      static_cast<double>(eq_acc) / static_cast<double>(total_eq);
  EXPECT_EQ(mc->get_iterations_count(), total_iterations);
  EXPECT_LE(initial_stepsize, uni->get_stepsize());
  EXPECT_DOUBLE_EQ(mc->get_nreject(),
                   mc->get_E_rejection_fraction() * mc->get_iterations_count());
...
TEST_F(AdaptiveTakeStepTest, Single_UniformAdaptive_Works) {
  const double initial_stepsize = 1e-2;
  auto uni = std::make_shared<mcpele::RandomCoordsDisplacementSingle>(
      42, nr_particles, box_dimension, initial_stepsize);
  const size_t total_iterations(2e5);
  const size_t equilibration_report_iterations(total_iterations / 2);
  mc->set_report_steps(equilibration_report_iterations);
  const size_t interval(equilibration_report_iterations / 1e2);
