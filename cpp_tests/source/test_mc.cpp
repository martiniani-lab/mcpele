...
TEST_F(TestMC, BasicFunctionalityAddingModulesStatic) {
  ...
  EXPECT_TRUE(k ==
              reinterpret_cast<pele::Harmonic *>(mc.get_potential_ptr().get())
                  ->get_k());
  shared_ptr<mcpele::RandomCoordsDisplacementAll> sampler_uniform =
      std::make_shared<mcpele::RandomCoordsDisplacementAll>(42, stepsize);
  EXPECT_TRUE(!mc.take_step_specified());
  mc.set_takestep(sampler_uniform);
  EXPECT_TRUE(mc.take_step_specified());
  shared_ptr<mcpele::MetropolisTest> metropolis =
...
TEST_F(TestMC, BasicFunctionalityPolyHarmonic) {
  ...
  EXPECT_TRUE(
      k ==
      static_cast<pele::Harmonic *>(mc->get_potential_ptr().get())->get_k());
  // std::cout << "initial step size: " << mc->get_stepsize() << std::endl;
  std::shared_ptr<mcpele::TakeStep> sampler_uniform_adaptive =
      std::make_shared<mcpele::AdaptiveTakeStep>(
          std::shared_ptr<mcpele::TakeStep>(
              new mcpele::RandomCoordsDisplacementAll(42)),
          50);
  EXPECT_TRUE(!mc->take_step_specified());
  ...
  shared_ptr<mcpele::Action> record_histogram =
      std::make_shared<mcpele::RecordEnergyHistogram>(0, 10, 1, adj_iter);
  mc->add_action(record_histogram);
  mc->run(max_iter);
  EXPECT_TRUE(mc->get_iterations_count() == max_iter);
  const double computed_mean =
      std::static_pointer_cast<mcpele::RecordEnergyHistogram>(record_histogram)
          ->get_mean();
  const double computed_std = sqrt(
      std::static_pointer_cast<mcpele::RecordEnergyHistogram>(record_histogram)
          ->get_variance());
...
class TrivialPotential : public pele::BasePotential {
 public:
  size_t call_count;
  virtual ~TrivialPotential() {}
  TrivialPotential() : call_count(0) {}
  double get_energy(Array<double> const &coords) {
    call_count++;
    return 0.;
  }
  double get_energy_change(Array<double> const &old_coords,
...
class TestMCMock : public ::testing::Test {
 public:
  ...
  virtual void TearDown() { delete mc; }
};
...
TEST_F(TestMCMock, PatternStep_Works) {
  ...
  std::vector<size_t> expected_pattern(
      std::accumulate(repetitions.begin(), repetitions.end(), 0));
  std::fill(expected_pattern.begin(), expected_pattern.begin() + repetitions0,
            0);
  std::fill(expected_pattern.begin() + repetitions0,
            expected_pattern.begin() + repetitions0 + repetitions1, 1);
  std::fill(expected_pattern.begin() + repetitions0 + repetitions1,
            expected_pattern.end(), 2);
  const auto actual_pattern = step_pattern->get_pattern();
  EXPECT_EQ(actual_pattern.size(), expected_pattern.size());
  EXPECT_EQ(actual_pattern.size(),
            std::accumulate(repetitions.begin(), repetitions.end(), 0u));
...
TEST_F(TestMCMock, AdaptiveTakeStep_WorksDown) {
  ...
  mcpele::RandomCoordsDisplacementAll *rs =
      new mcpele::RandomCoordsDisplacementAll(42, 1);
  auto astep = std::make_shared<mcpele::AdaptiveTakeStep>(
      std::shared_ptr<mcpele::RandomCoordsDisplacementAll>(rs), 100, 0.8);
...
TEST_F(TestMCMock, RecordEnergyHistogram_Works) {
  ...
  mcpele::RecordEnergyHistogram *hist_ =
      new mcpele::RecordEnergyHistogram(0, 42, 2, report_steps);
